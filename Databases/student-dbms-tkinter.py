...
def run_query(query,parameters=()):
    ...
    try:
        ...
        tkinter.messagebox.showerror("Database Error",str(error))
    finally:
        cur.close()
        conn.close()
    return query_result
...
def insert_data():
    query = f"insert into students(name, address, age, number) values (%s,%s,%s,%s)"
...
def delete_data():
    ...
    student_id = int(tree.item(selected_item)['values'][0])
    print(selected_item,tree.item(selected_item)['values'])
    query = f"delete from students where student_id=%s"
    parameters = (student_id,)
    run_query(query,parameters)
    tkinter.messagebox.showinfo("Information","Student record deleted successfully")
    refresh_treeview()
...
def update_data():
    ...
    run_query(query,parameters)
    tkinter.messagebox.showinfo("Information","Student record updated successfully")
    refresh_treeview()

def create_table():
    query = "create table if not exists students(student_id serial primary key,name text, address text, age int, number text);"
    run_query(query)
    tkinter.messagebox.showinfo("Information","Table Created successfully")
    refresh_treeview()
...
tree.column("student_id",anchor=CENTER,width=90)
